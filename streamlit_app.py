...
st.markdown(f"""
    ...
    /* FIXED: Format Card wraps content */
    .format-card {{
...
use_global = st.sidebar.toggle("Enable Global Compression", value=False)
    ...
    if not uploaded_files:
        ...
        for category, col in cats.items():
            with col:
                ...
                for spec in cat_specs:
                    ...
                    # FIXED: One container for the card background
        ...
        if st.button("🚀 GENERATE ALL BATCH ASSETS", use_container_width=True):
            if selected_formats:
                ...
                st.download_button("📂 DOWNLOAD ZIP", data=zip_buffer.getvalue(), file_name=f"{sanitize(project_name)}.zip", mime="application/zip")
...
with tab_lib:
    ...
    for idx, spec in enumerate(st.session_state.specs):
        with st.expander(f"✎ {spec['category']}: {spec['label']}"):
            ...
            c3, c4 = st.columns(2)
            e = c3.selectbox("Default Format", ["WebP", "JPEG"], index=0 if spec.get('ext', 'WebP') == "WebP" else 1, key=f"edit_e_{idx}")
            q = c4.slider("Default Quality", 10, 100, spec.get('quality', 80), key=f"edit_q_{idx}")
            
            # FIXED: Buttons on the same line
